        ...
        while True:
            ...
            if state == STATE_SEARCHING:
                ...
                if decision.cluster_found:
                    # 🛑 1. STOP CHASSIS
                    if not is_stopped:
                        print("  🛑 Grape detected! Stopping chassis...")
                        esp.stop()
                        is_stopped = True
                    ...
                    # ⬆️ DRIVE FORWARD
                    if is_stopped:
                        print("  ⬆️ No grapes found. Driving forward...")
            ...
            # ──────────────────────────────────────────────────────────────────
                ...
                if result.success and result.masks is not None:
                    ...
                    if cutting and cutting.primary:
                        ...
                        if time.time() - last_servo_send > 5.0 and abs(angle - last_sent_angle) >= 1:
                            # Step only ±1° at a time toward the target to reduce torque
                ...
                cv2.putText(vis, "VISUAL SERVOING (Press q to quit)", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
                cv2.imshow("ViniBot Pipeline", vis)
